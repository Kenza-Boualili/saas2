            ...
            <form onSubmit={vueAuth === 'connexion' ? gererConnexion : gererInscription} className="space-y-5">
              {vueAuth === 'inscription' && ( 
                <div className="relative">
                  ...
                  <Input placeholder="Nom de l'entreprise" className="pl-12 pr-4 bg-[#0a0f1d] border-slate-700 h-12 text-white rounded-xl text-sm focus:border-amber-500" value={nomEntreprise} onChange={(e) => setNomEntreprise(e.target.value)} required />
                </div> 
              )}
              <div className="relative">
                <Mail className="absolute left-4 top-3.5 w-5 h-5 text-slate-500" />
                <Input type="email" placeholder="Adresse e-mail" className="pl-12 pr-4 bg-[#0a0f1d] border-slate-700 h-12 text-white rounded-xl text-sm focus:border-amber-500" value={email} onChange={(e) => setEmail(e.target.value)} required />
              </div>
              <div className="relative">
                <Lock className="absolute left-4 top-3.5 w-5 h-5 text-slate-500" />
                <Input type="password" placeholder="Mot de passe" className="pl-12 pr-4 bg-[#0a0f1d] border-slate-700 h-12 text-white rounded-xl text-sm focus:border-amber-500" value={motDePasse} onChange={(e) => setMotDePasse(e.target.value)} required />
              </div>
              ...
              <Button type="submit" className="w-full h-12 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold mt-2 rounded-xl text-sm transition-colors shadow-lg shadow-amber-900/20">
                {vueAuth === 'connexion' ? 'Se connecter' : "S'inscrire"}
              </Button>
            </form>
            <div className="mt-8 text-center text-xs text-slate-400">
              {vueAuth === 'connexion' ? "Vous n'avez pas de compte ?" : "Vous avez déjà un compte ?"}
              <button onClick={() => { setVueAuth(vueAuth === 'connexion' ? 'inscription' : 'connexion'); setErreurAuth(''); }} className="ml-2 text-amber-400 hover:text-amber-300 font-semibold underline-offset-4 hover:underline">{vueAuth === 'connexion' ? "Créer un espace" : "Se connecter"}</button>
            </div>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="min-h-screen bg-[#0a0f1d] text-slate-50 flex font-sans relative overflow-hidden">
        ...
        <div className="fixed inset-0 bg-[#0a0f1d]/85 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <Card className="w-full max-w-lg bg-[#111827] border-slate-800 shadow-2xl rounded-2xl">
            <CardHeader className="flex flex-row items-center justify-between p-6 border-b border-slate-800/60">
              <CardTitle className="text-lg font-bold text-white flex items-center gap-2"><Plus className="w-5 h-5 text-amber-500"/> Nouvelle Intervention</CardTitle>
            ...
            <CardContent className="p-6">
              <form onSubmit={soumettreClientManuel} className="space-y-4">
                ...
                <div className="pt-3 flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setModalAjoutOuvert(false)} className="bg-transparent border-slate-700 text-slate-300 h-10 text-xs">Annuler</Button><Button type="submit" className="bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold h-10 text-xs">Enregistrer</Button></div>
              </form>
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent className="flex-1 overflow-hidden p-0 flex flex-col md:flex-row h-full">
              <div className="w-full md:w-1/3 border-r border-slate-800 p-5 space-y-4 overflow-y-auto bg-[#0a0f1d]/30">
                 ...
                 <div className="bg-[#0a0f1d] p-3.5 rounded-xl border border-slate-800 space-y-2.5">
                   <h4 className="text-[11px] font-bold text-amber-400 uppercase tracking-wider">Facturation & Devis</h4>
                   ...
                   <div className="pt-2 flex flex-col gap-1.5">
                     <Button onClick={() => telechargerDocumentPdf(prospectSelectionne.id, 'devis')} className="w-full bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs h-8 font-semibold"><Download className="w-3 h-3 mr-1" /> Devis PDF</Button>
                     ...
                     {messageSauvegarde && <span className="text-center text-[10px] text-amber-400">{messageSauvegarde}</span>}
                   </div>
                 </div>
              </div>
      ...
      <aside className="w-64 bg-[#0d1322] border-r border-slate-800/80 flex flex-col hidden lg:flex z-10 shrink-0">
        <div className="p-5 flex items-center gap-3 border-b border-slate-800/60">
          ...
          <div><h1 className="text-base font-extrabold tracking-tight text-white">KraftPilot</h1><span className="text-[10px] text-amber-400 font-semibold tracking-wider uppercase">Enterprise</span></div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 text-xs font-medium">
          <div>
            ...
            <nav className="space-y-1">
              <button onClick={() => setVueActuelle('dashboard')} className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl transition-all ${vueActuelle === 'dashboard' ? 'bg-amber-500 text-slate-950 font-bold shadow-lg shadow-amber-900/20' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white'}`}><LayoutDashboard className="w-4 h-4" /> Vue d'ensemble</button>
              <button onClick={() => setVueActuelle('finances')} className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl transition-all ${vueActuelle === 'finances' ? 'bg-amber-500 text-slate-950 font-bold shadow-lg shadow-amber-900/20' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white'}`}><Wallet className="w-4 h-4" /> Trésorerie</button>
            </nav>
          </div>
        ...
        {vueActuelle === 'clients' && (
          <div className="space-y-6 animate-in fade-in duration-300">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={() => setModalAjoutOuvert(true)} className="bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold text-xs h-10 px-4 rounded-xl"><Plus className="w-4 h-4 mr-2"/> Ajouter</Button>
            </div>
          ...
          <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-300">
            ...
            <form onSubmit={sauvegarderProfil} className="space-y-6">
              <Card className="bg-[#111827] border-slate-800 rounded-2xl p-6">
                <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2"><Building2 className="w-4 h-4 text-amber-500"/> Identité Pro</h3>
              ...
              <Card className="bg-[#111827] border-slate-800 rounded-2xl p-6">
                <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2"><Euro className="w-4 h-4 text-amber-500"/> Grille Tarifaire</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              ...
              <div className="flex items-center gap-4"><Button type="submit" className="bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold text-xs h-10 px-6 rounded-xl"><Save className="w-4 h-4 mr-2"/> Enregistrer</Button>{messageSauvegarde && <span className="text-xs text-amber-400 font-medium">{messageSauvegarde}</span>}</div>
            </form>
          </div>
        )}
          ...
          <div className="max-w-2xl mx-auto h-[75vh] flex flex-col animate-in fade-in duration-300">
            ...
            <Card className="flex-1 bg-[#111827] border-slate-800 flex flex-col overflow-hidden rounded-2xl shadow-xl">
              <CardContent className="flex-1 p-5 overflow-y-auto flex flex-col gap-3">
                {messages.map((m, idx) => (<div key={idx} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[80%] rounded-xl p-3 text-xs ${m.role === 'user' ? 'bg-amber-500 text-slate-950 font-medium' : 'bg-[#0a0f1d] border border-slate-800 text-slate-300'}`}>{m.content}</div></div>))}
              ...
              <div className="p-3 border-t border-slate-800 bg-[#0a0f1d]">
                <form onSubmit={envoyerMessage} className="flex gap-2"><Input value={nouveauMessage} onChange={(e) => setNouveauMessage(e.target.value)} placeholder="Simuler un message client..." className="flex-1 bg-[#111827] border-slate-700 text-white h-10 text-xs" /><Button type="submit" className="bg-amber-500 hover:bg-amber-600 text-slate-950 h-10 px-4 text-xs font-bold"><Send className="w-3.5 h-3.5"/></Button></form>
              </div>
            </Card>
          </div>
        )}
